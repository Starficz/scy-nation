...
public class StarficzAIUtils {
    ...
    public static class FutureHit{
        ...
        public String toString() {
            return String.format("EnemyID: %s | TimeToHit: %.2f | Damage: %.2f",
                    enemyId, timeToHit, damage);
        }
    }
    ...
    public static List<FutureHit> incomingProjectileHits(ShipAPI ship, Vector2f testPoint){
        ...
        while (iterator.hasNext()) {
            ...
            if (!(next instanceof DamagingProjectileAPI threat)) continue;
    ...
    public static List<FutureHit> generatePredictedWeaponHits(ShipAPI ship, Vector2f testPoint, float maxTime){
        ...
        for (ShipAPI enemy: nearbyEnemies) {
            ...
            if(enemy.getSystem() != null && enemy.getSystem().getSpecAPI().getDamageType() != null){
                ShipSystemSpecAPI systemSpec = enemy.getSystem().getSpecAPI();
                // if weapon out of range, add time for ship to reach you
                float distanceFromWeapon = MathUtils.getDistance(enemy.getLocation(), testPoint);
                float targetingRadius = Misc.getTargetingRadius(enemy.getLocation(), ship, false);
                float outOfRangeTime = ventOverloadTime;
                float actualRange = systemSpec.getRange(enemy.getMutableStats()) + 25f;

                if(distanceFromWeapon > (actualRange + targetingRadius) ){
                    Vector2f dirVec = VectorUtils.getDirectionalVector(enemy.getLocation(), testPoint);
                    float closingSpeed = enemy.getMaxSpeed() - Vector2f.dot(ship.getVelocity(), dirVec);
                ...
                while(currentTime < maxTime){
                    ...
                    futurehit.angle = VectorUtils.getAngle(testPoint, enemy.getLocation());
                    futurehit.damageType = systemSpec.getDamageType();
                    futurehit.softFlux = false;
                    futurehit.hitStrength = systemSpec.getDamage();
                    futurehit.damage = systemSpec.getDamage();
                    futurehit.empDamage = systemSpec.getEmpDamage();
                    futureHits.add(futurehit);
                    currentTime += 1f;
                }
            }
            ...
            for (WeaponAPI weapon: enemy.getAllWeapons()){
                ...
                float targetingRadius = Misc.getTargetingRadius(enemy.getLocation(), ship, false);
                float outOfRangeTime = ventOverloadTime;
                float actualRange = weapon.getRange() + weapon.getProjectileFadeRange()*0.8f + 25f;
                ...
                if(!(weapon.hasAIHint(WeaponAPI.AIHints.DO_NOT_AIM) || weapon.hasAIHint(WeaponAPI.AIHints.GUIDED_POOR))){
                    ...
                    if (targetPoint == null) aimTime = maxTime*2;
                        ...
                        float availableLimit = Math.abs(weapon.getArcFacing() - weapon.getCurrAngle() + weapon.getArc()/2 * rotationNeeded > 0 ? 1 : -1);
                        float noLimitTime = rotationNeeded/(weapon.getTurnRate() + enemy.getMaxTurnRate());
                        boolean hitLimit = weapon.getTurnRate()*noLimitTime > availableLimit;
                        if(!hitLimit) aimTime = Math.abs(rotationNeeded)/(weapon.getTurnRate() + enemy.getMaxTurnRate());
                        else {
                            float arcDistance = weapon.distanceFromArc(targetPoint);
                            aimTime = Math.abs(Math.abs(rotationNeeded) - arcDistance)/weapon.getTurnRate() + arcDistance/enemy.getMaxTurnRate();
                        }
                    }
                }
                ...
                */
                ...
                if(weapon.getSpec().getProjectileSpec() instanceof MissileSpecAPI){
                    ...
                    Vector2f dirVec = VectorUtils.getDirectionalVector(enemy.getLocation(), testPoint);
                    float globalSpeed = Vector2f.dot(ship.getVelocity(), dirVec);
                    float maxSpeed = stats.getMissileMaxSpeedBonus().computeEffective(missileEngine.getMaxSpeed()) - globalSpeed;
