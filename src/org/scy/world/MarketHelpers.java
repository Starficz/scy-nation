...
public class MarketHelpers {
    ...
    public static PersonAPI addPerson(
            ImportantPeopleAPI ip, MarketAPI market, String rankId, String postId, boolean noDuplicate) {
        ...
        if (postId.equals(Ranks.POST_BASE_COMMANDER)
                || postId.equals(Ranks.POST_STATION_COMMANDER)
    ...
    public static boolean addPersonSpecific(
            ImportantPeopleAPI ip,
            PersonAPI person,
            MarketAPI market,
            String rankId,
            String postId,
            boolean noDuplicate) {
        if (noDuplicate && hasPersonSpecific(market, person)) return false;
    ...
    public static void addMarketPeople(MarketAPI market) {
        ...
        if (market.getMemoryWithoutUpdate().getBoolean(MemFlags.MARKET_DO_NOT_INIT_COMM_LISTINGS))
            return;
        if (SCY_modPlugin.haveNexerelin) return;
    ...
    public static void generateMarketsFromEconJson(String fileName) {
        try {
            JSONObject economyJson =
                    Global.getSettings().loadJSON("data/campaign/econ/" + fileName + ".json");
            String starSystem = economyJson.getString("starSystem");
            generateMarketsFromEconJson(Global.getSector().getStarSystem(starSystem), fileName);
        } catch (Throwable ex) {
